...
public class BinarySearchSpan {
    ...
    public static void main(String[] args) {
        ...
        // forall i1: 1 <= i1 < a.length forall i2: i1 < i2 < a.length a[i1] <= a[i2])
        ...
        // (forall i1: 1 <= i1 < a.length forall i2: i1 < i2 < a.length a[i1] <= a[i2])
        int x_start = iterativeBinarySearch(x, a);
        int x_end = recursiveBinarySearch(x, a, -1, a.length);
        // args.length > 0 && forall i: 0 <= i < args.length args[i] is int && x == args[0] &&
        // forall i: 0 <= i < a.length a[i] = args[i + 1] &&
        // (forall i1: 0 <= i1 < pos1 a[i1] < x) &&
        // (forall i2: pos2 <= i2 < a.length a[i2] > x)
        System.out.println(x_start + " " + (x_end - x_start));
    }
    // args.length > 0 && (forall i: 0 <= i < args.length args[i] is int) &&
    // (forall i1: 1 <= i1 < args.length forall i2: i1 < i2 < args.length args[i1] <= args[i2])
}
